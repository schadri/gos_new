    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-12">
        {dynamicMetrics.map((item, i) => (
          <div key={i} className="bg-card dark:bg-zinc-900/50 p-6 rounded-3xl border border-border/50 shadow-sm dark:shadow-none hover:shadow-md hover:-translate-y-1 transition-all">
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <Link 
                          ...
                          className="text-xl font-bold text-foreground hover:text-primary transition-colors hover:underline decoration-primary/30 underline-offset-4"
                        >
                          {job.title}
                        </Link>
                        {job.status === 'active' ? (
                    ...
                    <div className="flex items-center gap-6 sm:gap-10">
                      <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-1">
                        ...
                        <Link 
                          href={`/employer/jobs/${job.id}/applicants`}
                          className="text-sm font-semibold text-primary/80 flex items-center hover:underline"
                        >
                          <Users className="h-4 w-4 mr-1.5" /> {job.applications_count || 0} aplic.
                        </Link>
                      </div>
                      <div className="flex items-center gap-4">
                        <Link href={`/employer/jobs/${job.id}/applicants`}>
                          <Badge variant="outline" className="px-3 py-1.5 bg-primary/10 text-primary border-primary/20 font-bold shadow-sm transition-colors hover:bg-primary/20 cursor-pointer">
